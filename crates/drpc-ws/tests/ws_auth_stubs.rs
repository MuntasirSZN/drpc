...
async fn ws_authorize_and_authenticate_error_stubs() {
    ...
    ws.send(tokio_tungstenite::tungstenite::Message::Text("{\"cmd\":\"AUTHORIZE\",\"nonce\":\"na\"}".into())).await.unwrap();
    let r1 = ws.next().await.expect("r1").expect("ok");
    let t1 = match r1 { tokio_tungstenite::tungstenite::Message::Text(t) => t, _ => panic!("expected text") };
    ...
    ws.send(tokio_tungstenite::tungstenite::Message::Text("{\"cmd\":\"AUTHENTICATE\",\"nonce\":\"nt\"}".into())).await.unwrap();
    let r2 = ws.next().await.expect("r2").expect("ok");
    let t2 = match r2 { tokio_tungstenite::tungstenite::Message::Text(t) => t, _ => panic!("expected text") };
