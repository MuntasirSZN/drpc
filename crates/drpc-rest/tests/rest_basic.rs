...
async fn rest_activity_and_metrics() {
    ...
    let port = drpc_rest::run_rest(bus.clone(), registry.clone().into(), None, 24, 0)
        .await
        .expect("rest");
    ...
    let list2 = client
        ...
        .await
        .unwrap();
    assert!(!list2["activities"]
        .as_array()
        .unwrap()
        .iter()
        .any(|e| e[1]["name"] == "BlockedGame"));
    // Clear
    client
        .delete(format!("{}/activities/{}", base, sid))
        .send()
        .await
        .unwrap();
}
